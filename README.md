...
包含**用户端**和**后台管理系统**，其中用户端面向广大用户，提供找房、看房预约、租约管理等功能，后台管理系统面向管理员，提供公寓（房源）管理、租赁管理、用户管理等功能。
...
本项目的核心业务流程为签约、续约、退租，在上述的业务流程中，会涉及到租约状态的多次变化，下面详细介绍一下租约状态。租约共有7个状态，分别是`签约待确认`、`已签约`、`已取消`、`已到期`、`退租待确认`、`已退租`、`续约待确认`。

**前端**：VUE3

**后端**：Spring Boot、Spring MVC、MyBatis、MyBatis Plus、MySQL、Redis、MinIO